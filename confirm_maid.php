...
else
{
    ...
    @$q="SELECT * FROM maid_bookings where customer_username='$username'";
    ...
    if ($w_username!=null)
    {
        ...
        if($res)
        {
            while ($row=mysqli_fetch_array($res))
            {
                ...
                if ($username==$dbusername && $w_username==$dbw_username && $service==$dbservice && $city==$dbservice_city && $area==$dbservice_area && $address==$dbservice_address && $pin==$dbservice_pin && $from==$dbfrom && $days==$dbdays && $time==$dbtime && $duration==$dbduration)
                {
                    $count=$count + 1;
                    break;
                }
            ...
            {
                $query="insert into maid_bookings(customer_username ,transportation_charge_day , worker_username,   service  ,  service_city , service_area, service_address , service_pin, from_date  ,  days ,  time  ,  duration ,   booking_date , worker_rate , transport_included, worker_cost, web_cost,pay_method,worker_card_no,user_card_no,total) values('$username','$transport_charge','$w_username','$service','$city','$area','$address','$pin','$from','$days','$time','$duration','$date','$rate','$transport_included','$worker_cost','$web_cost','$payment','$w_card_no','$u_card_no','$total')";
