...
else
{
    ...
    @$q="SELECT * FROM renovation_bookings where customer_username='$username'";
    ...
    if ($w_username!=null)
    {
        ...
        if($res)
        {
            while ($row=mysqli_fetch_array($res))
            {
                ...
                $dbtime=$row['time'];
                $dbhouse=$row['house_size'];
                if ($username==$dbusername && $w_username==$dbw_username && $dbfrom==$from && $time==$dbtime && $house==$dbhouse && $dbservice==$service && $dbservice_city==$city && $dbservice_area==$area && $dbservice_address==$address && $pin==$dbservice_pin)
                {
                    $count=$count + 1;
                    break;
                }
            ...
            {
                $query="insert into renovation_bookings(customer_username  , worker_username,   service  , house_size , service_city , service_area, service_address , service_pin, from_date  , time  , booking_date , worker_rate , worker_cost, web_cost,pay_method,worker_card_no,user_card_no,total) values('$username','$w_username','$service','$house','$city','$area','$address','$pin','$from','$time','$date','$rate','$worker_cost','$web_cost','$payment','$w_card_no','$u_card_no','$total')";
