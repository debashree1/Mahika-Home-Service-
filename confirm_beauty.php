...
else
{
    ...
    @$q="SELECT * FROM beauty_bookings where customer_username='$username'";
    ...
    if ($w_username!=null)
    {
        ...
        if($res)
        {
            while ($row=mysqli_fetch_array($res))
            {
                 ...
                 $dbtime=$row['time'];
                if ($username==$dbusername && $w_username==$dbw_username && $dbfrom==$from && $dbtime==$time && $dbservice==$service && $dbservice_city==$city && $dbservice_area==$area && $dbservice_address==$address && $pin==$dbservice_pin)
                {
                    $count=$count + 1;
                    break;
                }
