...
else
{
    ...
    @$q="SELECT * FROM tuition_bookings where customer_username='$username'";
    ...
    if ($w_username!=null)
    {
        ...
        if($res)
        {
            while ($row=mysqli_fetch_array($res))
            {
                ...
                if ($username==$dbusername && $w_username==$dbw_username && $service==$dbservice && $dbservice_city==$city && $dbservice_area==$area && $dbservice_address==$address && $dbservice_pin==$pin && $from==$dbfrom && $time==$dbtime && $dbduration==$dbduration && $dbmonths==$months && $level==$dblevel){
                    $count=$count + 1;
                    break;
                }
            }
            ...
            {
                $query="insert into tuition_bookings(customer_username,transportation_charge_month , level  , worker_username,   service  ,  service_city , service_area, service_address , service_pin, from_date  ,  months ,  time  ,  duration ,   booking_date , worker_rate , transport_included, worker_cost, web_cost,pay_method,worker_card_no,user_card_no,total) values('$username','$transport_charge','$level','$w_username','$service','$city','$area','$address','$pin','$from','$months','$time','$duration','$date','$rate','$transport_included','$worker_cost','$web_cost','$payment','$w_card_no','$u_card_no','$total')";
