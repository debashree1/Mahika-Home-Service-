...
else
{
    ...
    @$q="SELECT * FROM movers_bookings where customer_username='$username'";
    ...
    if ($w_username!=null)
    {
        ...
        if($res)
        {
            while ($row=mysqli_fetch_array($res))
            {
                ...
                if ($username==$dbusername && $w_username==$dbw_username && $dbfrom==$from && $time==$dbtime && $origin==$dborgin && $destination==$dbdestination && $dbservice==$service)
                {
                    $count=$count + 1;
                    break;
                }
            ...
            {
                $query="insert into movers_bookings(customer_username  , worker_username,   service  , origin , destination , from_date  , time  , booking_date , worker_rate , worker_cost, web_cost,pay_method,worker_card_no,user_card_no,total) values('$username','$w_username','$service','$origin','$destination','$from','$time','$date','$rate','$worker_cost','$web_cost','$payment','$w_card_no','$u_card_no','$total')";
